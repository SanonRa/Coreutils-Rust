...
fn main() {
    ...
    while i < args.len() {
        ...
        if arg == "--help" {
            ...
            if i + 1 >= args.len() {
                eprintln!("head: option requires an argument -- '{}'", arg);
                process::exit(1);
            }
            let val = args[i + 1].parse::<usize>().unwrap_or_else(|_| {
                eprintln!("head: invalid number: '{}'", args[i + 1]);
                process::exit(1);
            });
            if arg == "-n" {
    ...
    for file in &files {
        ...
        if let Some(n) = lines_count {
            ...
            let mut handle = reader.take(c as u64);
            let mut buffer = [0u8; 8192];
            while let Ok(read_bytes) = handle.read(&mut buffer) {
                if read_bytes == 0 {
                    break;
                }
                let _ = stdout.write_all(&buffer[..read_bytes]);
            }
        }
    }
}
