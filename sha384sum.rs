...
struct Sha384 { state: [u64; 8], count: u128, buffer: [u8; 128] }
impl Sha384 {
    fn new() -> Self {
        Self {
            state: [
...
fn main() {
    ...
    for arg in &args[1..] {
        match arg.as_str() {
            ...
            "--help" => {
                println!("Usage: sha384sum [OPTION]... [FILE]...\nPrint or check SHA384 (384-bit) checksums.\n\n  -b, --binary   read in binary mode\n  -c, --check    read SHA384 sums from the FILEs and check them\n  -t, --text     read in text mode (default)\n      --help     display this help and exit");
                return;
            }
            _ if arg.starts_with('-') && arg != "-" => { eprintln!("sha384sum: unrecognized option '{}'", arg); process::exit(1); }
    ...
    if check_mode {
        ...
        for file in files {
            let reader: Box<dyn Read> = if file == "-" { Box::new(io::stdin()) } else {
                match File::open(&file) { Ok(f) => Box::new(f), Err(e) => { eprintln!("sha384sum: {}: {}", file, e); continue; } }
            };
        ...
        if fail_count > 0 { eprintln!("sha384sum: WARNING: {} computed checksum did NOT match", fail_count); process::exit(1); }
    } else {
        ...
        for file in files {
            match hash_file(&file) {
                ...
                Err(e) => { eprintln!("sha384sum: {}: {}", file, e); exit_code = 1; }
            }
        }
