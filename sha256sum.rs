...
fn main() {
    ...
    for arg in &args[1..] {
        match arg.as_str() {
            ...
            _ if arg.starts_with('-') && arg != "-" => { eprintln!("sha256sum: unrecognized option '{}'", arg); process::exit(1); }
    ...
    if check_mode {
        ...
        for file in files {
            let reader: Box<dyn Read> = if file == "-" { Box::new(io::stdin()) } else {
                match File::open(&file) { Ok(f) => Box::new(f), Err(e) => { eprintln!("sha256sum: {}: {}", file, e); continue; } }
            };
            for line in BufReader::new(reader).lines().map_while(Result::ok) {
                if let Some(idx) = line.find(' ') {
                    let expected_hash = &line[..idx];
                    let target_file = line[idx..].trim_start_matches([' ', '*']).trim();
                    match hash_file(target_file) {
                        Ok(actual_hash) if actual_hash == expected_hash => println!("{}: OK", target_file),
                        _ => { println!("{}: FAILED", target_file); fail_count += 1; }
                    }
                }
            }
        }
        if fail_count > 0 { eprintln!("sha256sum: WARNING: {} computed checksum did NOT match", fail_count); process::exit(1); }
    } else {
        ...
        for file in files {
            match hash_file(&file) {
                ...
                Err(e) => { eprintln!("sha256sum: {}: {}", file, e); exit_code = 1; }
            }
        }
